      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className={`text-center mb-20 ${isAr ? "text-right" : ""}`}>
          ...
          >
            ...
            <span className="text-[#00E5A0] text-sm font-semibold uppercase tracking-widest">{t(h.sectionLabel)}</span>
            <span className="w-8 h-px bg-[#00E5A0]" />
          </motion.div>
          <motion.h2
        ...
        <div className="relative">
          {/* Connector line */}
          <div className={`absolute top-16 ${isAr ? "right-0" : "left-0"} right-0 h-px bg-gradient-to-r from-transparent via-[#00E5A0]/30 to-transparent hidden lg:block`} />
              ...
              >
                ...
                <div className="relative flex justify-center mb-6">
                  <div className="relative w-16 h-16 rounded-2xl bg-[#0A1A14] border border-[#00E5A0]/30 flex items-center justify-center group-hover:border-[#00E5A0]/60 transition-all duration-300 group-hover:shadow-[0_0_24px_rgba(0,229,160,0.2)]">
                    <span className="text-[#00E5A0] font-black text-lg">{step.num}</span>
                  </div>
                  {/* Arrow connector (desktop) */}
                  {i < h.steps.length - 1 && (
                    <div className={`absolute top-1/2 ${isAr ? "left-0 -translate-x-full" : "right-0 translate-x-full"} -translate-y-1/2 hidden lg:flex items-center`}>
                      <div className="w-8 h-px bg-[#00E5A0]/30" />
                      <div className={`w-0 h-0 border-t-4 border-b-4 border-transparent ${isAr ? "border-r-4 border-r-[#00E5A0]/30 rotate-180" : "border-l-4 border-l-[#00E5A0]/30"}`} />
                    </div>
                  )}
                </div>

                <h3 className="text-white font-bold text-lg mb-2">{t(step.title)}</h3>
                <p className="text-white/40 text-sm leading-relaxed">{t(step.desc)}</p>
              </motion.div>
            ))}
          </div>
        </div>
